...
class FeedDetailViewModel @Inject constructor(private val feedRepository: FeedRepository): ViewModel() {
    ...
    fun getCateDetail(catId: String?) {
        if (catId == null) {
            _feedDetailUiState.value = FeedDetailUiState.ShowError(IllegalStateException("Not a valid Id"))
            return
        }
        feedRepository.getCatDetail(catId).onEach { state ->
            when(state) {
                ...
                is NetworkState.Success -> _feedDetailUiState.value = FeedDetailUiState.ShowFeedDetail(state.data)
            }
